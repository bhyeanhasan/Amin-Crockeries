        ...
        if Cart.objects.filter(product=product, customer=user).exists():
            for crd in card:
                if (crd.product == product):
                    ...
                    Cart.objects.filter(product=product, customer=user).update(quantity=quantity,
                                                                               amount=price * quantity)
        else:
            card = Cart(product=product, customer=user, quantity=quantity, amount=price * quantity)
            card.save()
...
def deletefromcart(request, id):
    if request.user.is_authenticated:
        ...
        card = Cart.objects.get(customer=user, product=product)
        ...
        if card:
            if card.quantity <= 1:
                ...
                Cart.objects.filter(pk=card.pk).update(quantity=quantity, amount=price * quantity)
